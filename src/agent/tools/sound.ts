...
export const setSfx: ToolDef<{ n: number; speed: number; loop_start?: number; loop_end?: number; notes: { pitch: string | number; wave?: number; vol?: number; fx?: number }[] }> = {
  ...
  parameters: { type: 'object', properties: {
    ...
    notes: { type: 'array', items: { type: 'object', properties: { pitch: { type: 'string', description: 'note name like "c3", "f#2" (c0..d#5)' }, wave: { type: 'integer', minimum: 0, maximum: 7 }, vol: { type: 'integer', minimum: 0, maximum: 7 }, fx: { type: 'integer', minimum: 0, maximum: 7 } }, required: ['pitch'] } },
...
export const setMusic: ToolDef<{ pattern: number; channels: (number | null)[]; flags?: number }> = {
  ...
  description: 'Set music pattern (0-63): channels = 4 sfx numbers (-1 = channel off); flags: 1 loop start, 2 loop end, 4 stop.',
  parameters: { type: 'object', properties: { pattern: { type: 'integer', minimum: 0, maximum: 63 }, channels: { type: 'array', items: { type: 'integer', minimum: -1, maximum: 63 }, description: '4 entries: sfx 0-63 or -1 for off' }, flags: { type: 'integer', minimum: 0, maximum: 7 } }, required: ['pattern', 'channels'] },
  summarize: (a) => `#${a.pattern} ${JSON.stringify(a.channels)}`,
  run(a, ctx) {
    if (a.channels.some((v) => v !== null && (typeof v !== 'number' || v < -1 || v > 63))) return fail('channels: sfx numbers 0-63 or -1');
    const ch = [0, 1, 2, 3].map((i) => { const v = a.channels[i]; return v === null || v === undefined || v < 0 ? 0x41 + i : v & 0x3f; }) as [number, number, number, number];
